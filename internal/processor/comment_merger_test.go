...
func TestCommentMerger_FormatFileComment(t *testing.T) {
	...
	output := merger.FormatFileComment(fc, prCtx)
...
func TestCommentMerger_FormatWithLinks(t *testing.T) {
	...
	output := merger.FormatSummaryAddons(comments, prCtx)
	...
	if !strings.Contains(output, expectedFileLink) {
		t.Errorf("summary missing expected file link: %s", expectedFileLink)
	}
	if !strings.Contains(output, expectedLineLink) {
		t.Errorf("summary missing expected line link: %s", expectedLineLink)
	}
}
